...
public class UploadService extends Service
{
	...
	public int onStartCommand(Intent intent, int flags, int startId)
	{
		if (intent != null)
		{
			...
			UploadData data = intent.getParcelableExtra(UPLOAD_DATA);
	...
	private void init()
	{
		...
		m_builder.setColor(ContextCompat.getColor(this, R.color.color_primary))
				...
				.setTicker(getString(R.string.upload_content_notification_ticker))
				.setOngoing(true)
				.setAutoCancel(true);
	}
	...
	{
		PreferencesManager.getInstance().setIsUploading(UploadService.this, false);
		PathUtils.clearTempDirectory(UploadService.this, "upload");
	...
	private UploadData.ProgressListener m_listener = new UploadData.ProgressListener()
	{
		@Override
		public synchronized void onProgress(long uid, int progress)
		{
			...
			if (m_progressMap.size() <= 1)
			{
				...
				contentTitle = getString(R.string.upload_content_notification_ticker) + " (" + progress + "%)";
			}
			else
			{
				...
				contentTitle = String.format(getString(R.string.upload_content_notification), m_progressMap.size()) + " (" + totalProgress + "%)";
			}
		...
		public void onCompleted(long uid, boolean isSuccess, String errorMessage)
		{
			...
			if (m_progressMap.size() == 0)
			{
				if (isSuccess == true)
				{
					uploadCompleted(getString(R.string.upload_content_notification_complete_successed_title), getString(R.string.upload_content_notification_complete_successed_text));
				}
				else
				{
					uploadCompleted(getString(R.string.upload_content_notification_complete_failed_title), getString(R.string.upload_content_notification_complete_failed_text1));
				}
		...
		@Override
		public void onCompletedSameImage(long uid, boolean isSuccess, boolean isExists, List<String> tags, String errorMessage)
		{
